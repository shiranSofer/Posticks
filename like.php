...
<?php
    ...
    if(isset($_POST['like_button'])) {
        ...
        $total_user_likes++;
        if(!mysqli_query($connection, "UPDATE users SET num_likes='$total_user_likes' WHERE email='$posted_by'"))
            echo "UPDATE total_user_likes error: " . mysqli_error($connection);
        $update_user_likes_query = mysqli_query($connection, "UPDATE users SET num_likes='$total_user_likes' WHERE email='$posted_by'");
        if(!mysqli_query($connection, "INSERT INTO likes VALUES (NULL, '$user_email', '$post_id')"))
    ...
    if(isset($_POST['unlike_button'])) {
        if ($number_of_likes > 0) {
            ...
            $total_user_likes--;
            $update_user_likes_query = mysqli_query($connection, "UPDATE users SET num_likes='$total_user_likes' WHERE email='$posted_by'");
            $delete_like_query = mysqli_query($connection, "DELETE FROM likes WHERE liked_by='$user_email' AND post_id='$post_id'");
        }
    }
    $check_query = mysqli_query($connection, "SELECT * FROM likes WHERE liked_by='$user_email' AND post_id='$post_id'");
