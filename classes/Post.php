...
class Post
{
    ...
    public function submitPost($body, $user_to, $public, $image)
    {
        ...
        if (!($check_if_empty == "" && $image == "")) {
            ...
            if (!$insert_post_query = mysqli_query($this->connection, "INSERT INTO posts VALUES (NULL , '$body', 
                    '$posted_by', '$user_to', '$date_created', '$public', '$user_active', 'false', '0', '$image')"))
    ...
    {
        $str = "";
        if (!($data = mysqli_query($this->connection, "SELECT * FROM posts WHERE deleted='false' ORDER BY id DESC")))
        ...
        while ($row = mysqli_fetch_array($data)) {
            ...
            if (!($is_public == 'false' && $posted_by != $this->user_obj->getEmail())) { //check post's privacy
                ...
                //check if posted user has an active account
                ...
                if ($this->user_obj->isFriend($posted_by)) {
                    ...
                    if ($this->user_obj->getEmail() !== $user_details_row['email']) {
                        $button_friend_request = "<input type='submit' name='" . $posted_by_username . "' class='friends_button' value='remove friend' style=' position: relative;
                                                float: right; background-color: #ff00525e; margin-top: -20px;'>";
                    } else {
                    ...
                    if ($image_path != "") {
                        $image_div = "<div class='image_post'><a target='_top' href='$image_path'>
                                        <img src='$image_path' alt='uploaded_picture'></a>
                                    </div>";
                    } else {
                        $image_div = "";
                    }
